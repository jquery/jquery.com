...
jQuery 4.0.0 is compatible with jQuery UI 1.13.3 or older.
...
In jQuery 3.x, `require("jquery")` in a Node.js environment without a global `document` (e.g., when using JSDOM) returned a factory function that accepted a `window` argument. In jQuery 4.0, the CommonJS wrapper no longer does this. Instead, use the dedicated factory entry point:
...
A `jquery/factory-slim` entry point is also available. Alternatively, if you attach the JSDOM window to `globalThis` before importing jQuery, `require("jquery")` will continue to work.
